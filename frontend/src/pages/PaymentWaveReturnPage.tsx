...
export function PaymentWaveReturnPage() {
  const [searchParams] = useSearchParams();
  const ref = searchParams.get('ref');
  const waveError = searchParams.get('wave_error');
  ...
  useEffect(() => {
    ...
    (async () => {
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        try {
          const result = await api.confirmWavePayment(ref);
      ...
      if (!cancelled) {
        ...
        setErrorMessage(
          waveError === '1'
            ? 'Wave could not complete this attempt in the browser. If you were charged, keep this page’s reference and contact support — we may still confirm the payment shortly.'
            : 'Payment is still processing. You can close this page; the donation will show on the campaign once Wave confirms it.'
        );
      }
    })();
    ...
    <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 py-16">
      {phase === 'loading' && (
        <div className="text-center max-w-md">
          ...
          <p className="text-surface-600 text-sm">Talking to Wave and updating the campaign…</p>
        </div>
      )}
      ...
      {phase === 'success' && campaign && (
        <div className="text-center max-w-md">
          ...
          <p className="text-surface-600 mb-6">
            Your Wave payment was recorded for <strong>{campaign.title}</strong>
